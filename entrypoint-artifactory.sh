...
# Copy artifactory.config.import.yml file to skip setup wizard - by bujail
copyConfig () {
	cp /artifactory.config.import.yml ${ARTIFACTORY_DATA}/etc/ && chown artifactory:artifactory ${ARTIFACTORY_DATA}/etc/artifactory.config.import.yml && chmod 644 ${ARTIFACTORY_DATA}/etc/artifactory.config.import.yml
}
